...
# than a real assistant reply.  These end up in nova.db when the old LLM-based
...
_CORRUPT_PATTERNS: list[re.Pattern] = [
    ...
    re.compile(r"Nova'?s? Identity:.*Nova is presented as", re.IGNORECASE | re.DOTALL),
    re.compile(r"1\.\s+Nova'?s? Identity:", re.IGNORECASE),
    re.compile(r"Generative Capabilities:.*Nova can research", re.IGNORECASE | re.DOTALL),
    re.compile(r"You are Nova.*You are not a chatbot.*You are an operator", re.IGNORECASE | re.DOTALL),
    re.compile(r"# Nova — AI Chief of Staff", re.IGNORECASE),
    ...
    re.compile(r"By following these rules, Nova aims to", re.IGNORECASE),
    re.compile(r"conversational AI named Nova.*guidelines|guidelines.*conversational AI named Nova", re.IGNORECASE | re.DOTALL),
]
