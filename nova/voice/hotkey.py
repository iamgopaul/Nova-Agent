...
class PTTListener:
    ...
    def start(
        ...
        if not _is_accessibility_trusted():
            print(
                "\n[Nova] Push-to-talk disabled — Accessibility permission not granted.\n"
                "  To enable voice: System Settings → Privacy & Security → Accessibility\n"
                "  → add Terminal.app (or your terminal), then restart Nova.\n"
        ...
        try:
            ...
            print(f"\n[Nova] Hotkey listener error: {exc} — text input still works.\n")
