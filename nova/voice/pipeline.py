...
from nova.voice.audio_io import AudioRecorder
from nova.voice.hotkey import PTTListener
from nova.services.speaker_identity import SpeakerIdentityStore
from nova.voice.speaker_focus import SpeakerFocusManager
from nova.voice.stt import WhisperSTT
from nova.voice.tts import create_tts
...
class VoicePipeline:
    ...
    def start(self) -> None:
        ...
        try:
            ...
            print(
                f"\n[Nova] Hotkey listener failed to start: {exc}\n"
    ...
    def _process(self, pcm: bytes) -> None:
        ...
        if not accepted:
            # Ignore ambient/background speech that doesn't address Nova.
            return
        ...
        if self._ENROLL_RE.search(transcript):
            ...
            self._local_reply(
                "Got it. Voice training started. Say three short commands with 'Nova' at the start."
            )
            return
        ...
        if self._speaker_focus.has_profile:
            ...
            if not matched:
                ...
                if self._voice_mismatch_count >= 3:
                    self._voice_mismatch_count = 0
                    if self.on_error:
                        self.on_error("Voice lock was uncertain, so I allowed this turn. Say 'Nova learn my voice' to retrain.")
                else:
                    now = time.time()
            ...
            while not self._conversation_stop.is_set():
                ...
                try:
                    ...
                    if self._conversation_stop.is_set():
                        break
                    if self.on_status:
                        self.on_status("Sending audio to Nova")
                    self._process(pcm)
                finally:
                    self._busy.release()
        except Exception as exc:
    ...
    def _filter_for_wake_word(self, transcript: str) -> tuple[bool, str]:
        ...
        if self.on_error and (now - self._last_wakeword_hint_ts) > 6.0:
            ...
            self.on_error("Say 'Nova' in your request so I know it's for me.")
        return False, ""
        ...
        def run_listener() -> None:
            ...
            while not done.is_set() and not self._conversation_stop.is_set():
                ...
                if baseline_dbfs is None:
                    ...
                    # Adapt quickly at the start while Nova playback dominates input.
                ...
                if self._INTERRUPT_RE.search(heard):
                    ...
                    # Immediately reflect that Nova is listening again.
                    self._set_state("recording")
                    break

                # Fallback: if user starts talking while Nova is speaking,
    ...
    def _looks_like_echo(heard: str, current_response: str) -> bool:
        """Heuristic: ignore transcripts that are likely Nova's own playback."""
