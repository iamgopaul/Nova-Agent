...
from nova.attachments import build_attachment_context
...
_ECHO_ABORT_PATTERNS: list[re.Pattern] = [
    ...
    re.compile(r"Nova is presented as Isabella", re.IGNORECASE),
    re.compile(r"1\.\s+Nova'?s? Identity:", re.IGNORECASE),
    re.compile(r"# Nova\s*[-—]\s*AI Chief of Staff", re.IGNORECASE),
    ...
    re.compile(r"You are Nova.*You are not a chatbot", re.IGNORECASE | re.DOTALL),
    re.compile(r"# Identity\s*\nYou are Nova", re.IGNORECASE),
    ...
    re.compile(r"You are Nova — a real person named Isabella", re.IGNORECASE),
    ...
    re.compile(r"conversational AI named Nova.+(?:guidelines|These guidelines|various aspects)", re.IGNORECASE | re.DOTALL),
    re.compile(r"By following these rules, Nova aims to provide", re.IGNORECASE),
]
...
# Inline [Image: …] / [Image N: …] markers the LLM puts in image-only responses
# (these are for document generation only and must be stripped from chat text).
...
def _build_visual_query(user_message: str, ai_response: str) -> str:
    ...
    if len(caps_in_msg) >= 2:
        # Avoid returning pure noise like "Nova" alone
        candidate = " ".join(caps_in_msg[:3])
        noise = re.compile(
            r"^(Nova|Hey|Hi|Hello|Please|Can|Could|Would|Should|Tell|Show|Find|"
...
async def generate_suggestions(
    ...
    """
    Use Nova Spark (llama3.2:3b) to generate 3 contextual follow-up suggestions
    ...
    fast_model = orchestrator._fast_model  # noqa: SLF001 — Nova Spark
...
def _refine_music_prompt_from_response(response: str, fallback: str) -> str:
    """
    Extract the best MusicGen prompt from Nova's actual response text.
...
"""
    ...
    if use_web:
        system = f"""You are Nova Core — a visual layout assistant. Your job is to choose
...
"""
    ...
    try:
        ...
        print(f"[Chat] Nova Core essay image placement failed: {exc}", flush=True)
        return []
...
def _build_story_sections(response: str, user_message: str) -> list[dict]:
    ...
    from nova.server.routers.document import (
...
async def chat(
    ...
    async def run_orchestrator() -> None:
        ...
        try:
            ...
            if _music_prompt:
                # Refine the music prompt using what Nova actually described in her
            ...
            # 2. Nova Core (middle model) reads the essay and picks section indices +
            ...
            if _is_essay_mode:
                ...
                async def _fallback_fetch_all_web() -> list[str]:
                    """Heuristic one image per section when Nova Core placement fails."""
                ...
                if _essay_sections:
                    emit_status("Nova Core: choosing where to place images…")
                    ...
                    if _placements:
                        ...
                        # Nova Core returned nothing usable — fall back to heuristics
                        if _use_web_imgs:
