"""
Nova Document router — LLM-powered document generation.
...
from nova.server.dependencies import get_orchestrator
...
async def _apply_web_images_to_content(content: dict, web_image_urls: list[str]) -> None:
    ...
    *content* (one URL per section, in order).  Sections that already have
    ...
    applied = 0
    # Aligned with section indices (sparse placement from Nova Core)
...
async def _llm_image_placement_analysis(
    ...
    """
    Use Nova Core to analyze a story and identify the best positions for images.
...
async def generate_document_endpoint(
    ...
    Generate a document:
      ...
      1b. Otherwise call Nova Core to produce structured JSON content.
    ...
    if body.response and fmt == "txt":
        # Plain text: strip markdown syntax, preserve text and line breaks exactly
        print(f"[DocRouter] TXT — using assistant response directly ({len(body.response)} chars)", flush=True)
        content = {"title": "", "body": _strip_markdown(body.response)}
    elif body.response and fmt == "xlsx":
        # Excel: try to pull markdown tables from the response first
        parsed = _parse_markdown_tables(body.response)
        if parsed:
    ...
    elif body.response and fmt not in ("csv",):
        print(f"[DocRouter] Using assistant response directly ({len(body.response)} chars)", flush=True)
    ...
    if wants_imgs and _MAX_IMAGES.get(fmt, 0) > 0:
        if body.web_image_urls:
            ...
            if not sections_with_prompts and body.response and fmt not in ("xlsx", "csv"):
                # No markers found in the parsed content — ask Nova Core to analyze
                # the story and identify the best positions for images (multi-model pipeline).
                max_imgs = _MAX_IMAGES.get(fmt, 4)
                print(
                    f"[DocRouter] No image markers found — asking Nova Core to place {max_imgs} images…",
                    flush=True,
                )
                ...
                if placements:
                    ...
                    print(
                        f"[DocRouter] Nova Core placed {len(placements)} image(s) via LLM analysis",
                        flush=True,
                    )
