...
from nova.server.dependencies import get_memory
...
def list_facts(memory: MemoryStore = Depends(get_memory)) -> list[dict]:
...
def save_fact(
    ...
    memory.save_fact(body.key, body.value, body.source)
...
def delete_fact(
    ...
    deleted = memory.delete_fact(key)
    if not deleted:
...
def get_history(
    ...
    """
    Return up to *n* most recent messages (oldest first).  High default so the
    full thread reloads after restart; the UI + localStorage rich state still
    applies on the active session.
    """
    n = min(max(n, 1), 10_000)
    return memory.get_recent_turns(session_id, n)


@router.get("/sessions", response_model=list[SessionSummary])
def list_sessions(memory: MemoryStore = Depends(get_memory)) -> list[dict]:
...
def update_session(
    ...
    if body.title is not None:
        renamed = memory.rename_session(session_id, body.title)
        if not renamed:
            raise HTTPException(status_code=404, detail=f"Session '{session_id}' not found.")

    if body.folder is not None:
        moved = memory.move_session_to_folder(session_id, body.folder)
        if not moved:
            raise HTTPException(status_code=404, detail=f"Session '{session_id}' not found.")
...
def list_folders(memory: MemoryStore = Depends(get_memory)) -> list[dict]:
    return memory.list_folders()
...
def create_folder(
    ...
    created = memory.create_folder(body.name)
    if not created:
...
def delete_folder(
    ...
    deleted = memory.delete_folder(folder_name)
    if not deleted:
