"""
nova/services/knowledge_feed.py
────────────────────────────────
Background knowledge feed — automatically fetches trending news and web content
on a schedule and stores summaries in Nova's fact store, so she always has fresh
...
The model weights never change — but Nova's working knowledge is always current.
"""
...
_USER_AGENT = "Nova-KnowledgeFeed/1.0 (+https://github.com/nova-agent)"
...
async def run_knowledge_fetch(memory: MemoryStore, feeds: list[str] | None = None) -> None:
    """
    Fetch fresh knowledge from the web and store it in Nova's fact store.
