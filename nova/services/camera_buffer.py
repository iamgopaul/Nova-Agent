...
Nova prepends this to voice turns and text chat so the model sees what you are doing *now*,
...
def refresh_from_frame(session_id: str, _settings, image_bytes: bytes, face_store) -> None:
    ...
    try:
        from nova.services.body_detector import BodyDetection, detect as mp_detect
        from nova.services.frame_detection import detect_all as frame_detect_all
        ...
        try:
            ...
            print(f"[Nova] Live camera MediaPipe error: {exc}")
        ...
        if do_full:
            try:
                ...
                print(f"[Nova] Live camera full detector error: {exc}")
