...
class MacOSTTS:
    ...
    def _resolve_voice(self, requested: str) -> str:
        ...
        if short in voices:
            print(f"[GAIA] macOS voice '{requested}' not found — using '{short}'.")
            return short
        if "Samantha" in voices:
            print(f"[GAIA] macOS voice '{requested}' not found — using 'Samantha'.")
            return "Samantha"
        fallback = sorted(voices)[0]
        print(f"[GAIA] macOS voice '{requested}' not found — using '{fallback}'.")
        return fallback
...
class KokoroTTS:
    ...
    def __init__(
        ...
        try:
            ...
            print(f"[GAIA] Kokoro init failed: {exc} — using macOS say.")
    ...
    def _speak_once(self, text: str) -> None:
        ...
        try:
            ...
            print(f"[GAIA] Kokoro error: {exc} — falling back to say.")
            self._fallback.speak(text)
...
class ElevenLabsTTS:
    ...
    def __init__(
        ...
        if api_key:
            try:
                ...
                print(f"[GAIA] ElevenLabs init failed: {exc} — using macOS say.")
        ...
        try:
            ...
            for voice in voice_list:
                ...
                if name == preferred_name.lower():
                    ...
                    print(f"[GAIA] ElevenLabs voice set to '{preferred_name}'.")
                    return resolved
            print(
                f"[GAIA] ElevenLabs voice '{preferred_name}' not found — using configured voice ID."
            )
            return fallback_voice_id
        except Exception as exc:
            print(
                f"[GAIA] Failed to resolve ElevenLabs voice '{preferred_name}': {exc} — using configured voice ID."
            )
            return fallback_voice_id
    ...
    def _speak_once(self, text: str) -> None:
        ...
        try:
            ...
            print(f"[GAIA] ElevenLabs error: {exc} — falling back to say.")
            self._fallback.speak(text)
...
def create_tts(settings) -> "KokoroTTS | ElevenLabsTTS | MacOSTTS":
    ...
    if engine == "elevenlabs":
        ...
        if not api_key:
            print("[GAIA] No ELEVENLABS_API_KEY found — using macOS say.")
            return macos
