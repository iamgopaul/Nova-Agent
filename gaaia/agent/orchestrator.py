...
from gaaia.services.location import resolve_weather_location
from gaaia.services.response_cache import build_llm_cache
...
class Orchestrator:
    ...
    def _get_model_name(self, model_id: str) -> str:
        """Map model ID to friendly GAAIA name."""
        model_map = {
            self._model:         "GAIA Pro",
            self._fast_model:    "GAIA Spark",
            self._swift_model:   "GAIA Air",
            self._core_model:    "GAIA Core",
            self._heavy_model:   "GAIA Pro",
            self._code_model:    "GAIA Code",
            self._vision_model:  "GAIA Vision",
            self._mind_model:    "GAIA Mind",
            self._creative_model:"GAIA Creative",
            self._insight_model: "GAIA Insight",
            self._sage_model:    "GAIA Sage",
            self._chat_model:    "GAAIA Chat",
            self._logic_model:   "GAIA Logic",
            self._mini_model:    "GAIA Mini",
            self._star_model:    "GAIA Star",
            self._open_model:    "GAIA Open",
            self._quant_model:   "GAIA Quant",
            self._reason_model:  "GAIA Reason",
        }
        return model_map.get(model_id, "GAAIA")
    ...
    _HEAVY_SIGNALS: list[tuple[str, int]] = [
        # Strong composition (4pts) — must hit GAIA Pro threshold without relying on the JSON router
        *[(w, 4) for w in (
    ...
    # ── Document generation signals → GAIA Pro (best content quality)
    # The LLM writes the full document content; side-car formats the file.
    # Must use the most capable model for best prose/structure quality.
    _DOC_GENERATION_SIGNALS: list[tuple[str, int]] = [
        *[(w, 4) for w in (
            ...
            "generate both pdf", "generate both a pdf", "generate pdf and word",
        )],
    ]

    # ── Generation requests (image / music / diagram) → GAIA Core
    # These fire a side-car event; LLM writes a short confirmation sentence only.
    # Image/music don't need the Pro model — Core is fine for a one-liner.
    ...
    # ── Chart / data visualization signals → GAIA Insight (analytical + chart JSON)
    ...
    # ── Quantitative mathematics / computation → GAIA Quant ──────────────────
    ...
    # ── Deep reasoning / proofs / derivations → GAIA Reason ──────────────────
    ...
    # Conversational / personal questions → GAIA Core (balanced, tool-capable)
    ...
    def _auto_select_model(self, user_message: str) -> tuple[str, str]:
        ...
        Routing priority (highest to lowest):
          code (≥2)  → GAIA Code     — dedicated coding specialist
          heavy (≥4) → GAIA Pro      — deep research & reasoning only when justified
          creative   → GAIA Creative — writing, brainstorming, style
          chat       → GAIA Core     — conversational, personal, opinions
          quick      → GAIA Spark    — ultra-short factual one-liners
          general    → GAIA Core     — balanced fallback (tool-capable)
        """
        ...
        scores: dict[str, int] = {
            ...
            "chat": 0, "generation": 0, "doc_generation": 0, "chart": 0,
            "quant": 0, "reason": 0,
        }
        ...
        for keyword, pts in self._DOC_GENERATION_SIGNALS:
            if keyword in text:
                scores["doc_generation"] += pts
        ...
        # Document generation → GAIA Pro for best content quality.
        # The LLM writes the full document (essay/report/slides); Pro produces far better prose.
        if scores["doc_generation"] >= 4:
            return self._heavy_model, "doc_generation"

        # Image / music / diagram generation → GAIA Core.
        # These only need a short confirmation sentence; Core is more than sufficient.
        if scores["generation"] >= 4:
            return self._core_model, "generation"

        # Chart / data visualization → GAIA Insight (analytical, structured output for chart JSON)
        if scores["chart"] >= 4:
        ...
        # Proof / derivation / CoT reasoning → GAIA Reason (deepseek-r1 chain-of-thought)
        ...
        # GAIA Pro activates for any substantive query — quality over speed.
        ...
        # Conversational / personal → GAIA Core (fast, tool-capable, balanced)
        if scores["chat"] == best_score and scores["chat"] >= 2:
            return self._core_model, "chat"

        # Ultra-short factual one-liners → GAIA Spark
        if scores["quick"] >= 3:
            return self._fast_model, "quick"

        # General fallback — GAIA Core handles most everyday queries well.
        return self._core_model, "general"

    # ── LLM-based router ─────────────────────────────────────────────────────

    _ROUTER_PROMPT = """You are GAIA's routing brain. You pick the RIGHT model for the user's message.
...
- core    : Factual answers, live data / web lookups / news / weather / current events, image generation, music generation, diagram generation, short explanations that don't need depth. Core has tool access.
- pro     : **High-quality writing and deep intellectual work** — document generation (Word/PDF/Excel/PowerPoint), full essays on named subjects, in-depth analysis, research-style explanations, strategy breakdowns, nuanced advice. Use pro whenever the output needs to be excellent quality prose or a full document.
...
6. Document generation (Word, PDF, Excel, PowerPoint, CSV, report, presentation, slides, essay+download) → "pro"
7. Image / drawing / generate image → "core"
8. Music / song generation → "core"
...
Hard rules:
- **Document generation always gets "pro"** — the quality of the output matters, and Pro produces significantly better prose, structure, and content than Core.
- **Live data beats depth.** If the answer needs today's world, pick "core" even for heavy topics.
- **Default bias: smaller model.** When torn between air and core → prefer air for chat, core for info lookup. Between core and pro → prefer core unless there's explicit depth cue or document generation.
- **Brevity is a signal.** Short, casual messages (< 12 words) almost never need Pro (unless they're asking for a document).
...
- Keep the tone and style of the base response."""
    ...
    async def _tool_augmented_run(
        ...
        Parallel pipeline:
          ...
          - tool_agent      → GAIA Core (mistral:7b) fetches live data via tools
    ...
    async def run(
        ...
        # the model and bypass GAIA Core's router.
        ...
        # lightest tool-capable model (GAIA Core). Only go to GAIA Pro if the
    ...
    def _factual_lookup_intent(text: str) -> bool:
        ...
        only generative writing.  Matches mid-sentence (e.g. "hey gaaia can you
    ...
    def _build_options(self, model: str) -> dict[str, int | float]:
        ...
        if model == self._code_model:
            ...
            # GAIA Air: light chat — a touch cooler than the global default to cut rambling
            options["num_ctx"] = min(
