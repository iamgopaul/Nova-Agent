...
from gaaia.server.dependencies import get_memory, get_optional_user, get_orchestrator
from gaaia.server.schemas import VoiceResponse
from gaaia.services.body_detector import pick_frame_best_hands
from gaaia.services.camera_context import (
    SCENE_UNAVAILABLE,
    camera_who_line,
    hands_ground_truth_block,
)
from gaaia.services.camera_vision import describe_scene_frame
...
from gaaia.services.video_camera import jpeg_frames_from_video_bytes
from gaaia.services.speaker_identity import SpeakerIdentityStore, extract_declared_name
from gaaia.services.voice_personalization import compose_stt_prompt, build_whisper_prompt, refresh_stt_prompt
...
# Maps session_id → set of speaker_labels that GAAIA explicitly asked to introduce themselves.
...
def _stt_terms_with_wake_words(base_terms: list[str], wake_words: list[str]) -> list[str]:
    out: list[str] = []
    seen: set[str] = set()
    for t in [*(base_terms or []), *(wake_words or []), "GAAIA", "hey gaaia"]:
        s = str(t).strip()
        if not s:
            continue
        key = s.lower()
...
def _normalize_wake_homophones(transcript: str) -> str:
    ...
    for pat in start_map:
        if re.search(pat, text, re.IGNORECASE):
            text = re.sub(pat, r"\1hey gaaia ", text, count=1, flags=re.IGNORECASE).strip()
            break
...
def _mark_identity_asked(session_id: str, speaker_label: str | None) -> None:
    """Record that GAAIA just asked this speaker to introduce themselves."""
    key = speaker_label or "_anonymous_"
    with _identity_prompt_lock:
...
def _get_kokoro_pipeline(settings):
    ...
    if _kokoro_pipeline is None:
        ...
        print(f"[GAIA] Kokoro TTS ready — voice: {voice}")
...
def warmup_kokoro_inference(settings) -> None:
    ...
    try:
        ...
        for _ in pipeline("Hi.", voice=voice, speed=speed):
            pass
        print("[GAIA] Kokoro inference graph warm — first Read Aloud will be instant.", flush=True)
    except Exception as exc:
        print(f"[GAIA] Kokoro inference warm-up failed (non-fatal): {exc}", flush=True)
...
# Larger chunks give Kokoro enough synthesis time to stay ahead of playback,
# eliminating the inter-chunk gaps that cause choppy/unfluent delivery.
_MAX_TTS_CHUNK_CHARS = 300
...
async def transcribe_and_respond(
    ...
    """
    Accept an audio upload, transcribe it via Whisper, run the GAAIA agent,
    ...
    def _frame_metrics() -> tuple[str, str]:
        from gaaia.services.body_detector import detect as mp_detect
    ...
    if frame_bytes and wants_camera_context:
        try:
            ...
            print(f"[GAIA] Voice frame metrics error: {exc}")
    ...
    if frame_bytes and wants_camera_context:
        try:
            ...
            print(f"[GAIA] Utterance vision error: {exc}")
    ...
    if frame_bytes is not None:
        try:
            ...
            print(f"[GAIA] Face recognition error in voice turn: {exc}")
    ...
    # Continue the turn as anonymous so GAAIA can still answer naturally.
    ...
    if speaker_label:
        try:
            if _is_anonymous_speaker(speaker_label):
                ...
                memory.save_fact("last_speaker", speaker_label, source="voice-id")
                # Only persist a display name if this profile was created after GAAIA asked for an intro.
                if declared:
    ...
    if declared and speaker_label:
        # `declared` is only True when GAAIA had previously asked for this speaker's intro
    ...
    if _should_append_identity_prompt(
        ...
        # Record that GAAIA just asked so the next turn can save the name if they respond.
        _mark_identity_asked(sid, speaker_label)
...
def _split_complete_sentences(buffer: str) -> tuple[list[str], str]:
    ...
    # on comma/semicolon so GAAIA starts speaking quickly.
        ...
        try:
            ...
            if frame_bytes is not None:
                ...
                try:
                    ...
                    print(f"[GAIA] Stream frame metrics error: {exc}")
                ...
                try:
                    ...
                    print(f"[GAIA] Stream utterance vision error: {exc}")
            ...
            print(f"[GAIA] Voice stream error: {exc}")
...
async def synthesize_voiceover(
    ...
    if preferred_engine == "kokoro":
        ...
        try:
            ...
            print(f"[GAIA] Kokoro speak error: {exc} — falling back to macOS say.")
...
async def synthesize_stream(
    ...
    if tts_cfg.get("engine", "macos_say") != "kokoro":
        audio_bytes, media_type = _synthesize_macos_say(settings, text)
        # Frame with 4-byte little-endian length prefix so the client's streaming
        # protocol can read it as a single chunk (same framing as Kokoro chunks).
        framed = len(audio_bytes).to_bytes(4, "little") + audio_bytes
        return Response(content=framed, media_type="application/octet-stream",
                        headers={"Cache-Control": "no-store"})
    ...
    return StreamingResponse(
        ...
        headers={"Cache-Control": "no-store", "X-GAAIA-TTS": "kokoro-stream"},
    )
