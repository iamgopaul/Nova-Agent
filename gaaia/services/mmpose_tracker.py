...
If imports fail, GAAIA falls back to MediaPipe when ``hand_mmpose_fallback`` is true.
"""
...
def _get_inferencer(device: str):
    ...
    with _lock:
        ...
        try:
            ...
            if not _warned:
                print(f"[GAIA] MMPose hand inferencer ready (device={dev}).", flush=True)
                _warned = True
        except Exception as exc:
            _inferencer_failed = True
            if not _warned:
                print(f"[GAIA] MMPose unavailable ({exc}); use MediaPipe or install mmpose+mmcv.", flush=True)
                _warned = True
            return None
    return _inferencer
...
def detect_hands_mmpose(img_rgb, *, device: str = "cpu") -> list[tuple[Any, str, float]]:
    ...
    try:
        ...
        print(f"[GAIA] MMPose hand inference error: {exc}", flush=True)
        return []
