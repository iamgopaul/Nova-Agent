...
Used by `/camera/detect` and the voice pipeline so GAAIA sees the same structured labels
...
def _get_yolo():
    ...
    with _yolo_lock:
        ...
        try:
            ...
            print(f"[GAIA] YOLO loaded: {name}", flush=True)
        except Exception as exc:
            print(f"[GAIA] YOLO unavailable: {exc}", flush=True)
            _yolo_model = False
            _yolo_model_name = name
...
def detect_all(image_bytes: bytes, face_store) -> list[dict]:
    ...
    from gaaia.services.image_decode import bgr_from_bytes
    ...
    try:
        ...
        print(f"[GAIA] MediaPipe detect error: {exc}")
    ...
    if not has_face_detection:
        try:
            ...
            print(f"[GAIA] Fallback face detect error: {exc}")
    ...
    if yolo:
        try:
            ...
            print(f"[GAIA] YOLO detect error: {exc}")
