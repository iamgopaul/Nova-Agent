...
def _detect_hands_for_frame(img_rgb):
    ...
    if backend == "mmpose":
        from gaaia.services.mmpose_tracker import detect_hands_mmpose

        rows = detect_hands_mmpose(img_rgb, device=mp_dev)
        if rows or not mp_fallback:
            return rows
    from gaaia.services.hand_tracker import detect_hands_task_or_legacy
...
def detect(image_bytes: bytes) -> tuple[list[BodyDetection], str]:
    ...
    """
    from gaaia.services.image_decode import bytes_to_rgb
    ...
    with _lock:
        from gaaia.services.mediapipe_runtime import (
            get_face_detector,
            get_pose_landmarker,
        ...
        try:
            ...
            print(f"[GAIA] MediaPipe FaceDetector error: {exc}", flush=True)
        ...
        try:
            ...
            print(f"[GAIA] MediaPipe PoseLandmarker error: {exc}", flush=True)
