...
echo "[GAAIA] Memory: ${AVAIL_RAM_GB} GB available / ${TOTAL_RAM_GB} GB total"
...
echo "[GAAIA] Ollama: parallel=$OLLAMA_NUM_PARALLEL  max_models=$OLLAMA_MAX_LOADED_MODELS  flash_attn=$OLLAMA_FLASH_ATTENTION  ram_tier=$RAM_TIER"

# Prefetch MediaPipe hand_landmarker.task (~8 MB) so the first camera frame is instant.
echo "[GAAIA] Prefetching vision models..."
python "$ROOT_DIR/scripts/ensure_models.py" || echo "[GAAIA] Warning: model prefetch failed — will retry when the camera runs."
